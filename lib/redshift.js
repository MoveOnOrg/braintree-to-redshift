...
module.exports = class {
  ...
  async createdAtTimestamps(tableName) {
    /**
     * Disputes API doesn't allow searching on create_at for some reason, so
     * use received_date instead for that table.
     */
    const sql = {
      'disputes': SQL`
        SELECT
          min(date_trunc('minute', received_date)) AS min,
          max(date_trunc('minute', received_date)) AS max
    ...
    const [{ min, max }] = await this.client.query(sql[tableName], { raw: true });
