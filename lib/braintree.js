...
module.exports = class {
  ...
  async uploadToS3(responseObject, redshiftTimestamps, csvName) {
    ...
    const writePromise = async isMin => new Promise((resolve, reject) => {
      ...
      if (responseObject == 'disputes') {
        /**
         * Disputes API doesn't allow searching on createdAt for some reason.
         * receivedDate seems like the next best thing.
         */
        searchStream = this.client.dispute.search(search => (
          isMin ? search.receivedDate().min(max) : search.receivedDate().max(min)
        ));
      }
      else {
        searchStream = this.client.transaction.search(search => (
      ...
      const fieldMap = fieldMaps[responseObject];

      searchStream.on('data', (data) => {
        writer.write(fieldMap(data));
      });
